...
The primary caregiver can optionally share the child's vault read-only with active caregivers. Grandma does not need Nubo Plus to read what is shared with her. She sees the information prepared for that child, but she does not edit it.
...
- If Nubo Plus access ends, sharing stops and caregiver copies are removed after sync.
- If Nubo Plus access is restored later, the primary caregiver confirms before sharing resumes.
