...
This is the method that solves the caregiver problem specifically. It does not ask the nanny to learn anything. It does not ask grandma to remember a password. It does not ask either of them to open an app twelve times a day. It asks for one thing: tap the matching button when something happens.
...
| **Onboarding** | None | High (download, account, learn) | Almost none (tap the matching button) |
...
The caregiver does not need Nubo Plus, does not need to create an account, and does not need to download anything. They scan a QR code once, and their events appear in your timeline with their name attached. If [privacy matters to your family](/blog/privacy/nubo-privacy-architecture/), the sync is end-to-end encrypted: the server relays data it cannot read.
